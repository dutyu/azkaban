...
public class Utils {
    ...
    public static String getVarKey(String key) {
        String varStr = Utils.getVarStr();
        return varStr + key + varStr;
    }

    public static File getZipDir() {
        return new File(System.getProperty("java.io.zipdir"));
    }

    public static boolean zipExists(String projectName) {
        if (StringUtils.isEmpty(projectName))
            return false;
        String zipName = projectName + ".zip";
        File zipFile = new File(Utils.getZipDir(), zipName);
        return zipFile.exists();
    }

    public static void deleteZipFile(String projectName) throws IOException {
        if (StringUtils.isEmpty(projectName))
            return;
        String zipName = projectName + ".zip";
        File zipFile = new File(Utils.getZipDir(), zipName);
