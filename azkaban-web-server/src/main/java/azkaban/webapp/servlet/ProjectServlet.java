...
public class ProjectServlet extends LoginAbstractAzkabanServlet {
    ...
    private List getProcessedProjects(List<SimplifiedProject> simplifiedProjects) {
        return simplifiedProjects.stream().map(project -> {
            Map project_ = null;
            try {
                project_ = BeanUtils.describe(project);
                project_.put("createdTime", Long.valueOf((String) project_.get("createdTime")));
                project_.put("projectId", Integer.valueOf((String) project_.get("projectId")));
            } catch (IllegalAccessException e) {
                logger.error("transform project obj error !", e);
            } catch (InvocationTargetException e) {
                logger.error("transform project obj error !", e);
            } catch (NoSuchMethodException e) {
                logger.error("transform project obj error !", e);
            }
            if (!MapUtils.isEmpty(project_)) {
                project_.put("hasZipFile", Utils.zipExists(project.getProjectName()) ? "Y" : "N");
                return project_;
            } else {
                return project;
            }
