...
public class ProjectManagerServlet extends LoginAbstractAzkabanServlet {
    ...
    private InputStream getInputStream(File file) throws IOException {
        if (file == null || !file.exists() || file.isDirectory()) {
            throw new IOException("file not exists or file type error !");
        }
        InputStream in;
    ...
    private void ajaxHandleUpload(final HttpServletRequest req, final HttpServletResponse resp,
        ...
        if (this.lockdownUploadProjects && !UserUtils
            ...
            try {
                ...
                params.put("type", type);
                params.put("variables", (String) multipart.get("variables"));

                final Map<String, ValidationReport> reports = uploadFile(user, item.getInputStream(), params);
    ...
    private Map<String, ValidationReport> uploadFile(User user, InputStream in, Map<String, String> params) throws SchedulerException, IOException {
        ...
        try {
            ...
            final File zipDir = Utils.getZipDir();
            final File archiveFile = new File(zipDir, zipFileName);
            ...
            final File archiveFile2 = generateArchiveFile(archiveFile, variables);
            logger.info("archiveFile2 path: " + archiveFile2.getAbsolutePath());
            ...
            final Map<String, ValidationReport> reports =
                    this.projectManager.uploadProject(project, archiveFile2, type, user,
                            props);

            if (!archiveFile.getAbsolutePath().equals(archiveFile2.getAbsolutePath())) {
                FileUtils.deleteDirectory(archiveFile2.getParentFile());
            }
    ...
    private List<Entry<String, String>> validateAndParseVar(String vairables) {
        ...
        return new ArrayList<>(kvMap.entrySet());
    }


    private void traverseAndReplaceVars(File tempFile, List<Entry<String, String>> kvList) throws IOException {
        if (tempFile.exists()) {
            ...
            if (files.length == 0) {
                ...
                for (File file : files) {
                    if (file.isDirectory()) {
                        ...
                        try {
                            bufIn = Files.newBufferedReader(file.toPath());
                            tempStream = new CharArrayWriter();
                            String line;
                            while ((line = bufIn.readLine()) != null) {
                                for (Entry<String, String> entry : kvList) {
                                    String varKey = Utils.getVarKey(entry.getKey());
                                    line = line.replaceAll(varKey, entry.getValue());
                                }
                        ...
                        logger.info("replace file finish :" + file.getAbsolutePath());
                    }
                }
            }
        }
    ...
    private File generateArchiveFile(final File archiveFile, final String variables) throws IOException {
        ...
        try {
            ...
            List<Entry<String, String>> kvList = validateAndParseVar(variables);
            tempDir = Utils.createTempDir();
            File file = new File(Utils.createTempDir(), "temp.zip");
            Utils.unzip(new ZipFile(archiveFile), tempDir);

            logger.info("create and unzip the temp file, file path: " + tempDir.getAbsolutePath());

            traverseAndReplaceVars(tempDir, kvList);
